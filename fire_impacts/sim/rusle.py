...
def compute_klscp_layer(
    ...
    if use_fire_adjusted:
        ...
        k_factor_path = ctx.event_path('Erodibility', 'K_factor_adjusted.tif')
    else:
...
def _rusle_parameter_grids(
    ...
    klscp, klscp_meta = compute_klscp_layer(
        ctx, use_fire_adjusted=use_fire_adjusted,
    )

    transform = klscp_meta['transform']
    crs = klscp_meta['crs']

    # SDR and masked dNBR are per-event.
    sdr, _ = read_raster(ctx.event_path('Delivery', 'SDR.tif'))
    dnbr = read_aligned(
...
def lumped_daily_rusle(ctx: RunContext, rainfall):
    """
...
def run_usle_simulation(
    ...
    params = _rusle_parameter_grids(ctx, use_fire_adjusted=use_fire_adjusted)
    klscp, sdr, dnbr, cell_area_ha, transform = params

    # Get the catchment boundary geometry in a form rasterio can use
    catchment_boundary = ctx.project.catchment_boundary(ctx.catchment)
    ...
    if save_rasters:
        ...
        for output_raster in c.RUSLE_OUTPUT_RASTER_NAMES:
            ...
            save_catchment_raster(
                ...
                out_path=os.path.join(
                    run_results_dir, f'{output_raster}.tif'),
            )
        ...
        aggregate_rusle_to_subcatchments(
            ctx, results_section=results_section,
        )
