...
def _rusle_parameter_grids(project:FireImpactsProject, catchment:str):
    ...
    #generated by severity.calculate_fire_severity():
    dnbr = read_aligned(
        project.catchment_path(
            catchment,
            'FireSeverity',
...
def record_subcatchment_timeseries(proj:FireImpactsProject, variable_name: str,fn="sum", label_field=None,agg_count=1):
    '''
    Build a RUSLE recorder that summarizes a variable over subcatchments
    Parameters:
    Parameters:
    - proj (fire_impacts.FireImpactsProject): Current project
    - variable_name (str): Name of the variable to summarize.
    - fn (str): Summary function to use: 'sum', 'mean', 'max'. Default is 'sum'.
    - label_field (str): OPTIONAL: Field in the catchment boundaries to use as labels. If None, use the index.
    - agg_count (int): OPTIONAL: Number of timesteps to aggregate before recording. Default is 1.

    Note:
    - While the time series can always be resampled to a desired timestep AFTER the simulation, the agg_count parameter
      can be used to speed things up significantly by aggregating (sum) over multiple timesteps during the simulation itself.

          Returns:
    - timeseries_recorder (function): A recorder function that can be used in the simulation (see: run_usle_simulation).
    '''
    result = None
    index = None
    zones = None
    ...
    def timeseries_recorder(timestep,catchment,transform,**kwargs):
        nonlocal result, zones, zone_names, index, intermediate, intermediate_count
        data = kwargs.get(variable_name)
        if data is None:
            raise ValueError(f"Variable {variable_name} not found in simulation data.")

        if zones is None:
            boundaries_v = proj.catchment_boundary(catchment)#.to_crs(epsg=4326)
            zones = [rasterio.features.rasterize([g], transform=transform, fill=np.nan, dtype=np.float32, out_shape=data.shape) for g in boundaries_v.geometry]
    ...
    def reset():
        nonlocal result, index, zones, zone_names, intermediate, intermediate_count
        index = None
        zones = None
        zone_names = None
        result = None
...
def run_usle_simulation(project:FireImpactsProject, rainfall, catchment=None, recorders=None):
    '''
    Run the USLE simulation for a given project and rainfall data, recording results as specified.

    Parameters:
    - project (fire_impacts.FireImpactsProject): Current project
    - rainfall (Series-like): 30 minute rainfall data (mm)
    - catchment (str): Name of the catchment to process. If None, process all catchments.
    - recorders (dict): OPTIONAL: Dictionary of recorder functions to use during the simulation.

    Notes:
    - Each recorder function should accept parameters (timestep, **data) and return recorded results.
    - The data dictionary passed to each recorder will contain keys such as 'RUSLE', 'SDR', etc.
    - Each recorder function should also have 'reset' and 'finalize' methods to manage state.

    Returns:
    - results (dict): Dictionary of recorded results from the simulation.
    - params (tuple): Tuple of parameters used in the simulation (klscp, sdr, dnbr, cell_area_ha, transform).
    '''
    if catchment is None:
        return project.for_each_catchment(lambda c: run_usle_simulation(project,rainfall,c,recorders))
    ...
    catchment_boundary = project.subcatchment_boundaries(catchment).dissolve()
    ...
    print('Values in mask',np.unique(mask))
    klscp_masked = klscp*mask
    sdr_masked = sdr*mask
    dnbr_masked = dnbr*mask
    results = dict()
    for ts_data in generate_rusle(rainfall,klscp_masked, sdr_masked, dnbr_masked, cell_area_ha):
        timestep, data = ts_data
        for key, recorder in recorders.items():
            results[key] = recorder(timestep, **data,catchment=catchment,transform=transform)
...
def generate_rusle(rainfall:pd.Series, klscp:np.array, sdr:np.array, dnbr:np.array, cell_area_ha:float):
    ...
    Yield:
    - Tuple with timestep and dict with results:
      (timestep (datetime), dict with keys:
        'total_rain' (float), 'intensity' (float), 'erosivity' (float),
        'RUSLE' (np.array), 'SDR' (np.array),
